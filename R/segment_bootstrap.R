...
seg_bootstrap_iranges <- function(seg, x,state,L_b,chrnames, chrom_lens, 
 ...
 ## is half of the chromosome, then the block width is half of L_b
  ...
  suppressWarnings(random_blocks <- GRanges(seqnames = seqnames[index],ranges = random_blocks_r, seqlengths = chrom_lens)) ## warning message out-of-bound when end(seg)[j]-L_b+1<0
  suppressWarnings(random_blocks$key.random<-seq_len(length(random_blocks)))

  fo <- plyranges::join_overlap_inner(x,random_blocks) # Question1: do we want to only select once for each gene? Question2: Join_overlap_inner or join_overlap_intersect
