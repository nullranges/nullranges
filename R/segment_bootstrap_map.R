#' Segmentated bootstrap GRanges, map all chromosome to one line
#'
...
#'
#' @export
seg_bootstrap_granges_map <- function(seg, x, within_chrom = TRUE, L_b, proportion_length = TRUE, coarse = FALSE) {
  # type <- match.arg(type)
  ...
  if (within_chrom) {
    ...
    seg_length <- seg %>%
      ...
      summarise(Ls = sum(width)) # derive each states length
    L_s <- seg_length$Ls
    r_prime <- seg_bootstrap_iranges_map(new_seg, new_x, seg$state, L_c, L_s, L_b, proportion_length, coarse)
