...
seg_bootstrap <- function(x, seg, state, L_b,
  ...
  if (proportion_length) {
    ...
    L_s <- unlist(lapply(esses, function(s)
      sum(width(seg)[state == s])), use.names = FALSE)
    ...
    obj <- lapply(esses, function(s) { 
      ...
      random_rng <- sample(length(seg_s), n, replace = TRUE,
                           prob = width(seg_s))
      ...
      return(list(random_start_s, random_chr_names_s,
