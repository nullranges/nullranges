...
x <- DHSA549Hg38
system.time({
  x_prime <- segBootstrapRanges(x, seg, L_b, R = 10,
...
## system.time({
##   x_prime <- segBootstrapRanges(x, seg, L_b, R = 10, deny = deny)
## })
system.time({
  x_prime <- segBootstrapRanges(x, seg, L_b, R = 10, deny = deny,
...
```{r}
deny <- GRanges(c("chr1","chr2"), IRanges(c(21,32),c(40,50)), seqlengths=seqlengths(gr))
# this gives me an error if proportion_length=TRUE
gr_prime <- nullranges::segBootstrapRanges(gr, seg, L_b = 100,
                                           R = 1, deny = deny,
                                           within_chrom = FALSE,
