...
seg <- segmentDensity(g, n = 3, L_s = L_s,
                          deny = deny2, type = "cbs")
plotSegment(g,seg,L_s,deny2)
```

Note here the default ranges plot gives whole genome and cannot visualize state transformations within small ranges, e.g 1kb. If user want to look into specific ranges of segmentation state, the *region* argument is flexible to change. 

```{r}
region=GRanges("chr2",IRanges(9.4e7,9.5e7))
plotSegment(g,seg,L_s,deny2,type="ranges",region=region)
```
...
seg_hmm <- segmentDensity(g, n = 3, L_s = L_s,
                          deny = deny2, type = "hmm")
plotSegment(g,seg_hmm,L_s,deny2)
```
