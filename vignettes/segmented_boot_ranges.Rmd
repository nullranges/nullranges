...
We perform some processing to align these features with our excluded
regions and our features of interest (DNase hypersensitive sites, or
DHS, defined below).
...
## Import excluded region

We next import excluded regions (as created by
\citet{encode_exclude}). 
...
We use a CBS segmentation as implemented in *DNAcopy* \citep{dnacopy}.
...
These steps are included in *nullrangesOldData* in the
`scripts/DHSA549Hg38.R` script.
...
We prepare the objects below, note that we must add seqlengths for
block bootstrapping, as we need to create tiles along the genome. 
...
Here, we test the speed of the various options for bootstrapping. Note
that we use the `exclude` object from the previous step, which does
not contain small ranges. If one wanted to also avoid generation of
bootstrapped features that overlap small excluded ranges, then omit
...
microbenchmark(
  list=alist(
    prop = bootRanges(x, seg, blockLength, 1, exclude, proportionLength = TRUE),
    no_prop = bootRanges(x, seg, blockLength, 1, exclude, proportionLength = FALSE)
), times=10)
```
...
bootstrap, visualized with *plotgardener*.

Define a helper function for plotting *GRanges* using
*plotgardener*. A key aspect here is that we color the original and
