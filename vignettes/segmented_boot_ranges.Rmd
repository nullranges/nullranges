...
Test on 100 times bootstrap ~13s

```{r eval=FALSE}
L_b <- 5e5
system.time({
  res_wochr <- segBootstrapRanges(seg, pks, L_b, deny, R = 10, proportion_length = FALSE)
})
system.time({
  res_wochr_propl <- segBootstrapRanges(seg, pks, L_b, deny, R = 10)
})
system.time({
  res_wchr <- segBootstrapRanges(seg, pks, L_b, deny, R = 10,
