...
microbenchmark(list=alist(
                 prop = bootRanges(x, seg, L_b, 1, deny, proportion_length = TRUE),
                 no_prop = bootRanges(x, seg, L_b, 1, deny, proportion_length = FALSE)
), times=10)
```
...
```{r toy-no-prop}
gr_prime <- bootRanges(gr, seg, L_b = 25, R = 1, 
...
```{r toy-prop}
gr_prime <- bootRanges(gr, seg, L_b = 50, R = 1, 
