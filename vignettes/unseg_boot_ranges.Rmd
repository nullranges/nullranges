...
microbenchmark(list=alist(
                 p_within=bootstrap_granges(pks, L_b=L_b, within_chrom=TRUE, type="permute"),
                 b_within=bootstrap_granges(pks, L_b=L_b, within_chrom=TRUE),
                 p_across=bootstrap_granges(pks, L_b=L_b, within_chrom=FALSE, type="permute"),
                 b_across=bootstrap_granges(pks, L_b=L_b, within_chrom=FALSE)
), times=10)
```
...
for (i in 1:2) {
  gr_prime <- bootstrap_granges(gr, L_b=100, type="permute", within_chrom=TRUE)
...
for (i in 1:2) {
  gr_prime <- bootstrap_granges(gr, L_b=100, within_chrom=TRUE)
  plotGRanges(gr_prime)
}
```
