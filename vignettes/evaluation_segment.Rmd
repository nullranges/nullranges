...
# gr <- keepStandardChromosomes(gr,pruning.mode="coarse",species = "Homo_sapiens")
...
```
```{r}
seg<-segment_density(g1,n=3,Ls=1e6,type = "HMM",plot_origin = TRUE)
seg
L_b<-2e5
g1<-sort(g1)
system.time(res<-seg_bootstrap_granges(seg,g1,L_b=L_b))[[3]]
system.time(res2<-seg_bootstrap_granges(seg,g1,L_b=L_b,within_chrom = FALSE))[[3]]
system.time(res3<-seg_bootstrap_granges2(seg,g1,L_b=L_b))[[3]]
system.time(res4<-seg_bootstrap_granges2(seg,g1,L_b=L_b,within_chrom = FALSE))[[3]]
autoplot(res)
autoplot(res2)
autoplot(res3)
autoplot(res4)
...
x <- GRanges("1",IRanges(c(21,26,31,36,41,221,231,241),width=4))
library(ggbio)
autoplot(x) + xlim(1,400)
res <- do.call(c, seg_bootstrap_iranges(seg,x,L_c=400,L_s=c(200,100,100),L_b=100))
autoplot(res) + xlim(1,400)
```
```{r}
