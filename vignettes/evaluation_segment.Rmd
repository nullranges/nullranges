...
```{r}
Ls=1e6
seg<-segment_density(gr,n=3,Ls=Ls,deny=deny,type = "CBS",boxplot = TRUE)
```
...
# L_c<-seqlengths(g1)
# L_s<-seg_length$Ls
system.time(res_wchr <- seg_bootstrap_granges(seg,gr,L_b,within_chrom=TRUE,coarse = TRUE))[[3]]
system.time(res_ochr <- seg_bootstrap_granges(seg,gr,L_b,within_chrom=FALSE,coarse = TRUE))[[3]]

res_ochr<-sort(res_ochr)
res_ochr
