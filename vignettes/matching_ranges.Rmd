...
output:
  rmarkdown::html_document:
    ...
    fig_height: 3
vignette: |
  %\VignetteIndexEntry{1. Overview of matchRanges}
...
supported matching methods. To see `matchedRanges()` used in real
...
replacement. For more information see the section on [Choosing the
...
Results from `matchRanges` can also be used
in conjunction with `cobalt` for assessing covariate balance. For an example
see [Using `cobalt` to assess balancing](#using_cobalt).
...
finding the point at which the difference in focal and pool densitites
