...
## Case study I: Bootstrapping DHS from ENCODE
...
regions can be selected from *ExperimentHub*. We find that the
segmentation and block length shown here could be used for most
analysis of hg38. 
...
don't need for the bootstrap analysis. Because we are interested in
signal value for DHS peaks later, we only keep it here. Consider, when
creating 
...
## Case study II: Bootstrapping peaks count matrix in single-cell multi-omics
