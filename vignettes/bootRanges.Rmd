...
# remove non-standard chromosomes and mitochondrial genome
...
Note that we must remove non-standard chromosomes and mitochondrial
genome, as these are typically shorter than our desired `blockLength`.
...
promoter peaks over iterations and compare to the observed
statistic. The histogram below shows that our observed value falls far
...
If one is interested in assessing significance of gene-promoter pairs
per gene, one can also do `plyranges::select` (*gene*, *peak*, *rho*)
and `plyranges::group_by` *gene*.

* tidySummarizedExperiment:

Create an RangedSE for each modality, then find the overlap using the
