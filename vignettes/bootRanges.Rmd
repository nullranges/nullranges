...
As we said in previous, unsegmented is sufficient if the purpose is
for testing association between a smaller set of sequences rather than
genome-wide, such as transcripts, then the unsegmented algorithm would
be sufficient. The following chunk of code evaluates various types of 
bootstrap/permutation schemes, first segmented, then unsegmented
within chromosome, and finally unsegmented across 
...
Overall, schemes of across chromosome are fastest and are
preferred. Segmented and unsegmented have similar running time. For
the unsegmented bootstrap, the block bootstrap will be preferred over
block permutation, as the latter provides additional variance with
respect to e.g. the total number and repetition of individual input
ranges per iteration. 
