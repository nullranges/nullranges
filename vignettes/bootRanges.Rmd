...
The optional `seg` and `exclude` arguments improve bootstrap quality by
respecting large-scale genome structure and avoiding placing features in
inaccessible regions. Pre-processing steps to prepare these inputs, and
to prepare `ranges` itself, are shown below:

```{r eval=FALSE}
# seg: pre-built CBS segmentation for hg38 from ExperimentHub
eh <- ExperimentHub()
seg <- eh[["EH7307"]]

# exclude: excluded regions (centromeres, telomeres, etc.)
# see the excluderanges package, or load a BED file:
exclude <- plyranges::read_bed("exclude.bed")

# ranges: input ranges require seqlengths
seqlengths(ranges) # check; if missing see ?Seqinfo
# remove non-standard chromosomes and mitochondrial genome
# (chrM is typically shorter than blockLength)
...
There are two options for choosing a segmentation, either (1) use an exiting segmentation (e.g. ChromHMM, etc.) downloaded from AnnotationHub or external to Bioconductor (BED files imported with *rtracklayer*); (2) 
perform a *de novo* segmentation of the genome using feature density,
e.g. gene density.

**Pre-built segmentations**

Given that genome segmentation evaluations take time and involve
consideration of multiple criteria, we have provided our recommended
segmentation for hg38. Pre-built segmentations using either *CBS* or
*HMM* methods with $L_s=2e6$ considering excludable regions can be
downloaded directly from *ExperimentHub*. We find that the CBS
segmentation and block length (500kb) shown in the case study below
could be used for most analyses of hg38.
...
Users with a different genome can generate a segmentation for their own
purposes using gene annotations obtained via the *ensembldb* package
[@ensembldb; @ensembl2021]. The `segmentDensity()` function supports
CBS segmentation [@dnacopy] (`type = "cbs"`) or HMM segmentation via
*RcppHMM* (`type = "hmm"`), and `plotSegment()` can be used to
visualize the resulting segmentation states. Or segmentation can be
performed based on any large scale genomic features, such as isochores.
